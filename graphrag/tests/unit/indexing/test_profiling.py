...
class TestWorkflowProfiler:
    ...
    def test_memory_metrics_disabled(self):
        """Memory fields are zero when tracemalloc profiling is disabled."""
        with WorkflowProfiler() as profiler:
            _data = [0] * (1024 * 1024 // 8)
    ...
    def test_all_metrics_populated(self):
        """Verify metrics struct is returned with timing populated."""
        with WorkflowProfiler() as profiler:
            _ = list(range(100))
