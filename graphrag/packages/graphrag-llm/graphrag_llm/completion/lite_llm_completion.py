...
class LiteLLMCompletion(LLMCompletion):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._completion, self._completion_async, _cache_key_stable = (
        ...
        # 缓存键在 with_cache 阶段只看到外层 kwargs（多为 messages），不含已在
        # _base_completion 内合并的 max_tokens 等；若不并入，改 call_args 仍会命中旧缓存。
        cache_key_creator_wrapped = _wrap_cache_key_creator(
            self._cache_key_creator, _cache_key_stable
        )
        ...
        self._completion, self._completion_async = with_middleware_pipeline(
            ...
            request_type="chat",
            cache=self._cache,
            cache_key_creator=cache_key_creator_wrapped,
...
def _wrap_cache_key_creator(
    inner: Callable[[dict[str, Any]], str],
    stable: dict[str, Any],
) -> Callable[[dict[str, Any]], str]:
    """Merge LiteLLM 稳定请求参数，使缓存键随 max_tokens / temperature 等变化。"""

    def wrapped(kwargs: dict[str, Any]) -> str:
        return inner({**stable, **kwargs})
...
def _create_base_completions(
    *,
...
) -> tuple["LLMCompletionFunction", "AsyncLLMCompletionFunction", dict[str, Any]]:
    ...
    cache_key_stable = {
        k: v for k, v in base_args.items() if k not in _CACHE_KEY_EXCLUDE
    }
    ...
    return (_base_completion, _base_completion_async, cache_key_stable)
